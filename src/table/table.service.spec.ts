...
describe('TableHandlerService', () => {
  ...
  describe('createTable', () => {
    ...
    it('should throw error if table already exists', async () => {
      ...
      await expect(service.createTable(createTableDto))
        .rejects.toThrow('Table new_table already exists!');
    });
    ...
    it('should throw error if no id column with isPrimary', async () => {
      ...
      await expect(service.createTable(invalidDto))
        .rejects.toThrow('Table must contain a column named "id" with isPrimary = true.');
    });
    ...
    it('should throw error if id column has invalid type', async () => {
      ...
      await expect(service.createTable(invalidDto))
        .rejects.toThrow('The primary column "id" must be of type int or uuid.');
    });
    ...
    it('should throw error if multiple primary columns', async () => {
      ...
      await expect(service.createTable(invalidDto))
        .rejects.toThrow('Only one column is allowed to have isPrimary = true.');
    });
  ...
  describe('updateTable', () => {
    ...
    it('should throw error for non-existent table', async () => {
      ...
      await expect(service.updateTable(999, updateDto as any))
        .rejects.toThrow('Table undefined does not exist.');
    });
    ...
    it('should throw error if no primary column in update', async () => {
      ...
      await expect(service.updateTable(1, invalidUpdateDto as any))
        .rejects.toThrow('Table must contain an id column with isPrimary = true!');
    });
  ...
  describe('delete', () => {
    ...
    it('should handle foreign key constraints before dropping', async () => {
      ...
      expect(mockQueryRunner.query).toHaveBeenCalledWith(
        expect.stringContaining('ALTER TABLE `other_table` DROP FOREIGN KEY `FK_ref_test`')
      );
    ...
    it('should throw error for non-existent table', async () => {
      ...
      await expect(service.delete(999))
        .rejects.toThrow('Table with id 999 does not exist.');
    });
  ...
  describe('afterEffect', () => {
    ...
    it('should unlock schema even if sync fails', async () => {
      ...
      await expect(service.afterEffect({ entityName: 'test_table', type: 'update' }))
