...
export class FileAssetsService {
  ...
  private readonly maxCacheAge = { small: 3600, medium: 1800, large: 600, xlarge: 300 };
  ...
  constructor(
    ...
    if (!this.redis) this.logger.warn('Redis not available - image caching disabled');
  ...
  private async configureRedis(): Promise<void> {
    ...
    try {
      ...
      else this.logger.log(`Redis: ${(parseInt(maxMemory[1]) / (1024 * 1024)).toFixed(0)}MB, ${policy[1]}`);
      
      if (policy[1] === 'noeviction') this.logger.warn('Consider allkeys-lru policy');
    } catch (error) {
  ...
  async streamFile(req: RequestWithRouteData, res: Response): Promise<void> {
    ...
    if (!fileId) return void res.status(400).json({ error: 'File ID is required' });
    ...
    if (this.isImageFile(mimetype, fileType) && this.hasImageQueryParams(req)) {
      return void await this.processImageWithQuery(filePath, req, res, filename);
    }
  ...
  private async processImageWithQuery(
    ...
    try {
      ...
      const width = query.width ? parseInt(query.width as string, 10) : undefined;
      const height = query.height ? parseInt(query.height as string, 10) : undefined;
      const quality = query.quality ? parseInt(query.quality as string, 10) : undefined;
      const cache = query.cache ? parseInt(query.cache as string, 10) : undefined;

      const cacheKey = this.generateCacheKey(filePath, format, width, height, quality);
      ...
      if (cachedImage) {
        ...
        if (cache && cache > 0) res.setHeader('Cache-Control', `public, max-age=${cache}`);
        return void res.send(cachedImage.buffer);
      }

      await this.incrementStats('misses');

      if (width && (width < 1 || width > 4000)) return void res.status(400).json({ error: 'Width 1-4000' });
      if (height && (height < 1 || height > 4000)) return void res.status(400).json({ error: 'Height 1-4000' });
      if (quality && (quality < 1 || quality > 100)) return void res.status(400).json({ error: 'Quality 1-100' });
      ...
      if (format) {
        ...
        if (!supportedFormats.includes(format.toLowerCase())) {
          return void res.status(400).json({ error: `Unsupported format: ${supportedFormats.join(', ')}` });
        }
        imageProcessor = this.setImageFormat(imageProcessor, format.toLowerCase(), quality);
      } else if (quality) {
        const originalFormat = path.extname(filePath).toLowerCase().slice(1);
        imageProcessor = this.setImageFormat(imageProcessor, originalFormat, quality);
      }
      ...
      if (processedBuffer.length < 10 * 1024 * 1024 && frequency >= this.minHitsToCache) {
      ...
      if (!res.headersSent) res.status(500).json({ error: 'Image processing failed' });
    }
  }

  private setImageFormat(imageProcessor: sharp.Sharp, format: string, quality = 80): sharp.Sharp {
    const formatMap = {
      jpeg: () => imageProcessor.jpeg({ quality, progressive: true, mozjpeg: true, trellisQuantisation: true, overshootDeringing: true, optimizeScans: true }),
      jpg: () => imageProcessor.jpeg({ quality, progressive: true, mozjpeg: true, trellisQuantisation: true, overshootDeringing: true, optimizeScans: true }),
      png: () => imageProcessor.png({ quality, compressionLevel: 9, progressive: true }),
      webp: () => imageProcessor.webp({ quality, effort: 4, smartSubsample: true }),
  ...
  private async streamRegularFile(filePath: string, res: Response, filename: string, mimetype: string): Promise<void> {
  ...
  private getMimeType(format: string): string {
    const types = { jpeg: 'image/jpeg', jpg: 'image/jpeg', png: 'image/png', webp: 'image/webp', avif: 'image/avif', gif: 'image/gif' };
  ...
  private async checkFilePermissions(file: any, req: RequestWithRouteData): Promise<void> {
    ...
    const hasAccess = (file.permissions || []).some((p: any) => 
      p.isEnabled !== false && (p.allowedUsers?.id === user.id || (p.role && user.role?.id === p.role.id))
    );

    if (!hasAccess) throw new AuthorizationException('Access denied');
  }

  private generateCacheKey(filePath: string, format?: string, width?: number, height?: number, quality?: number): string {
    const stats = fs.statSync(filePath);
    return crypto.createHash('md5').update(`${filePath}-${stats.mtime.getTime()}-${format || 'original'}-${width || 0}x${height || 0}-q${quality || 80}`).digest('hex');
  }

  private async getFromCache(key: string): Promise<{ buffer: Buffer; contentType: string } | null> {
    ...
    try {
      ...
      return { buffer: bufferData, contentType: JSON.parse(metaData).contentType };
    } catch (error) {
  ...
  private async addToCache(key: string, buffer: Buffer, contentType: string): Promise<void> {
    if (!this.redis) return;
    try {
      ...
      pipeline.setex(`${this.cachePrefix}${key}:meta`, ttl, JSON.stringify({ contentType, size, timestamp: Date.now() }));
  ...
  private async logCacheStats(): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Cache: ${hitRate}% hit rate, ${hotKeysCount} hot keys, ${(currentMemory / 1024 / 1024).toFixed(0)}MB`);
  ...
  private async addToHotKeys(key: string, size: number): Promise<void> {
    ...
    try {
      ...
      pipeline.hset(`${this.hotKeysKey}:meta`, key, JSON.stringify({ size, timestamp: Date.now() }));
      await pipeline.exec();
    } catch (error) {
  ...
  private async cleanupLeastUsedCache(): Promise<void> {
    ...
    try {
      ...
      const candidates: Array<{ key: string; frequency: number; size: number; timestamp: number }> = [];
      ...
      candidates.sort((a, b) => a.frequency !== b.frequency ? a.frequency - b.frequency : a.timestamp - b.timestamp);
      ...
      if (evictCount > 0) {
        await pipeline.exec();
        this.logger.log(`Evicted ${evictCount} items, freed ${(freedMemory / 1024 / 1024).toFixed(2)}MB`);
      }
    } catch (error) {
  ...
  private async getCurrentCacheMemory(): Promise<number> {
    ...
    try {
      ...
      const metaDataArray = await this.redis.hmget(`${this.hotKeysKey}:meta`, ...hotKeys);
      let totalMemory = 0;
